...
sub breed_new_generation() {
	...
	my $index = $elites[$i];
	&printgd("breed(): Copying genome $index to the new population at index $i:\n");
...
sub mutate_genome() {
	...
	if(rand(1) < $mutation_rate) {
		...
		&crash("mutate(): Unrecognized connection type ($type).\n");
	    }
	}
...
sub write_analog_configuration() {
	...
	if($type eq $NULL) { 
	    ...
	    &crash("write_analog_configuration(): Unsupported connection type recieved ($type).\n");
	}
    }
}
